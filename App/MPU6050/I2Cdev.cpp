...
int8_t I2Cdev::readBytes(uint8_t devAddr, uint8_t regAddr, uint8_t length,
    ...
    uint8_t count = 0;
    uint32_t t1 = millis();
    I2C_HandleTypeDef *useWire = &hi2c1;
    if(wireObj){
        useWire = (I2C_HandleTypeDef*) wireObj;
    }

    int step = length < I2CDEVLIB_WIRE_BUFFER_LENGTH ? length : I2CDEVLIB_WIRE_BUFFER_LENGTH;
    for(int k = 0; k < length; k += step){
        HAL_I2C_Master_Transmit(useWire, devAddr, &regAddr, 1, I2CDEV_DEFAULT_READ_TIMEOUT);
        int size = (length - k) < I2CDEVLIB_WIRE_BUFFER_LENGTH ? (length - k) : I2CDEVLIB_WIRE_BUFFER_LENGTH;
        HAL_I2C_Master_Receive(useWire, devAddr, data, size, I2CDEV_DEFAULT_READ_TIMEOUT);
    }
...
int8_t I2Cdev::readWords(uint8_t devAddr, uint8_t regAddr, uint8_t length,
    ...
    int step = length < I2CDEVLIB_WIRE_BUFFER_LENGTH ? length : I2CDEVLIB_WIRE_BUFFER_LENGTH;
    for(uint8_t k = 0; k < length * 2; k += step){
        HAL_I2C_Master_Transmit(useWire, devAddr, &devAddr, 1, I2CDEV_DEFAULT_READ_TIMEOUT);
        HAL_I2C_Master_Receive(useWire, devAddr, (uint8_t*)data, length, I2CDEV_DEFAULT_READ_TIMEOUT);
    }
...
bool I2Cdev::writeBytes(uint8_t devAddr, uint8_t regAddr, uint8_t length,
    ...
    HAL_StatusTypeDef status = HAL_I2C_Master_Transmit(useWire, devAddr, &regAddr, length, I2CDEV_DEFAULT_READ_TIMEOUT);
...
/*
 ...
 are variables local to the I2C_HandleTypeDef object, and by the time we actually have read the
...
uint8_t I2C_HandleTypeDef::rxBuffer[NBWIRE_BUFFER_LENGTH];
uint8_t I2C_HandleTypeDef::rxBufferIndex = 0;
uint8_t I2C_HandleTypeDef::rxBufferLength = 0;

uint8_t I2C_HandleTypeDef::txAddress = 0;
uint8_t I2C_HandleTypeDef::txBuffer[NBWIRE_BUFFER_LENGTH];
uint8_t I2C_HandleTypeDef::txBufferIndex = 0;
uint8_t I2C_HandleTypeDef::txBufferLength = 0;

//uint8_t I2C_HandleTypeDef::transmitting = 0;
void (*I2C_HandleTypeDef::user_onRequest)(void);
void (*I2C_HandleTypeDef::user_onReceive)(int);
